...
async def register(user_in: UserCreate, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)) -> Any:
    ...
    # Logic: Admins or certain roles might be auto-verified, but default to False
    is_ver = False
...
async def login_access_token(
    ...
    if is_locked:
        ...
        raise HTTPException(status_code=429, detail=f"Too many failed attempts. Account locked for {minutes} more minute(s).")
...
async def update_doctor_profile(user_id: int, request: DoctorProfileUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Update a doctor's medical profile. Allowed by admin or the doctor themselves."""
...
async def list_doctors(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """List all doctors with full profiles."""
